...
| 時間 | やること | 成果物 | 状態 |
|------|----------|--------|------|
| 45分 | 動作確認（必須パス） | 1) 初回起動 2) 登録→復習 3) エクスポート/インポート 4) 設定で生年月日・入試日 5) サンプルデータ読み込み | 手動確認推奨 |
| 15分 | index 同期とコミット | `cp revenge-note.html index.html`。CHANGELOG 追記。commit & push。 | ✅ 済（v5.5.1 で実施） |

### 明日 午後（実施済み or 手順のみ）

| 時間 | やること | 成果物 | 状態 |
|------|----------|--------|------|
| 20分 | Supabase 確認 | `user_data` の RLS を確認。未設定なら「user_id = auth.uid()」でポリシー追加。 | 手順を `docs/SUPABASE-RLS.md` に記載済み。ダッシュボードで要実施。 |
| 30分 | 軽い UI 確認 | iPad または DevTools で今日・登録・一覧・履歴・分析・設定を一通り開く。 | 手動確認推奨 |
| 20分 | デプロイと本番確認 | push 済み。https://ppxa.github.io/revnote/ で必須パスを1回ずつ実行。 | push 済。本番確認は手動。 |
